...
| **[Dashboard Overtime Index Charts](./features/dashboard-overtime-index-charts.md)**                       | [Epic-09](../../scrum/Epic-09.md) | BurnUpIndexChart, updated CategoryOvertimeChart (plan vs actual grouped bars), YtdOvertimeIndexChart, Index multipliers (HKN×1.5/HLR×2.0), and Ytd2026DummyDataSeeder. |
...
| **[Overtime Planning OT (Monthly Grid)](./features/overtime-planning-ot.md)**                              | Transition                        | Monthly per-employee, per-day, per-category planning grid with DRAFT/PUBLISHED lifecycle, scoped RBAC, and operational calendar integration.                           |
| **[SPL Excel Import (Realized Overtime)](./features/spl-excel-import.md)**                                 | Transition                        | Upload `spl_manual_ot.xlsx`, parse per-day sheets, upsert realized overtime entries using NPK + date + start-time composite key; admin/manager gate.                   |
