...
| **[Executive Operational Dashboard & KPI Cards](./guides/executive-dashboard-kpi.md)**                 | Managers, Admins & TL   | Live plant command center (/dashboard), 4 header KPI cards (Production Volume, Working Days, Manpower, Burn Index), sparklines, and department/section/date filters.                                                  |
...
| **[Overtime Planning OT](./guides/overtime-planning-ot.md)**                                           | Team Leaders & Managers | Monthly overtime planning grid — schedule hours by employee, day, and category (Production / TPM / Project / Others) with DRAFT/PUBLISHED lifecycle.                                                                  |
| **[Input Lembur — SPL Upload](./guides/spl-excel-import.md)**                                          | Admins & Managers       | Upload `spl_manual_ot.xlsx` to import realized overtime records. Upserts on NPK + date + start-time. Admin can view and delete individual entries.                                                                    |
